...
Haxeはオープンソースの高級プログラミング言語とコンパイラから構成されており、ECMAScript\footnote{http://www.ecma-international.org/publications/standards/Ecma-327.htm}をもとにした構文で書いたプログラムをさまざまなターゲットの言語へとコンパイルすることを可能にします。適度な抽象化をしてやれば、複数のターゲットへコンパイル可能な1つのコードベースを作成できます。
...
\Fullref{class-field}では、Haxeのクラスの構造に関するに全てをあつかいます。加えて、\emph{プロパティ}, \emph{インラインフィールド}, \emph{ジェネリック関数}についてもあつかいます。

\Fullref{expression}では、\emph{式}を使用して実際に動作をさせる方法をお見せします。
