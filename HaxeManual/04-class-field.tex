...
\begin{description}
	...
	\item[プロパティ:] \tref{プロパティ}{class-field-property}クラスフィールドはアクセスされた時の独自の動作を定義します。クラスの外からは変数のフィールドのように見えます。
	\item[メソッド:] A \tref{メソッド}{class-field-method}はコードを実行するために呼び出すことのできる関数です。
\end{description}
