...
\end{enumerate}
ルール\ref{valueanytype} は複雑にネストした構造体を含む。例えば、以下のような。
\todo{please reformat}
...
\emph{文字列の定数値}すべてが許容されるますが、フィールドが\tref{Haxeの識別子}{define-identifier}として有効である場合のみ型の一部として認識されます。そして、Haxeの構文では識別子として無効なフィールドにはアクセスできないため、\tref{リフレクション}{std-reflection}の\expr{Reflect.field}と\expr{Reflect.setField}を使わなくてはいけません。
